...
//     case CHAR_XVC:
//         rv =
//             kobject_set_name(&xcdev->cdev.kobj, devnode_names[type],
//             xdev->idx);
//         break;
//     case CHAR_EVENTS:
//         rv = kobject_set_name(&xcdev->cdev.kobj, devnode_names[type],
...
void xpdev_destroy_interfaces(struct xdma_pci_dev *xpdev) {
    ...
    for (i = 0; i < xpdev->h2c_channel_max; i++) {
        ...
        rv = sha256_destory(&xpdev->sha256[i], g_xdma_class);
        if (rv < 0)
            pr_err("Failed to destroy sha256u%dc%d error :0x%x\n",
                   xpdev->xdev->idx, i, rv);
    }
    ...
    // 		if (rv < 0)
    // 			pr_err("Failed to destroy bypass h2c cdev %d error
    // 0x%x\n", 				i, rv);
    // 	}
    // 	for (i = 0; i < xpdev->c2h_channel_max; i++) {
...
int xpdev_create_interfaces(struct xdma_pci_dev *xpdev) {
    ...
    for (i = 0; i < xpdev->h2c_channel_max; i++) {
        ...
        rv = sha256_create(xpdev, &xpdev->sha256[i], i, engine, g_xdma_class);
        if (rv < 0) {
...
int xdma_cdev_init(void) {
    ...
    cdev_cache = kmem_cache_create("cdev_cache", sizeof(struct cdev_async_io),
